...
// Include new Avatar Manager
//#include "AvatarManager.h"
//Incldue new scene and model manager
...
static bool MainLoop(bool retryCreate)
{
	...
	//Create and initialize Avatar Client (sets up avatar SDK)
    ...
    while (Platform.HandleMessages())
    {
		...
		//Prerendering Avatar (pump avatar messages and update avatar)
		avatarClient->Prerender(session);
		
		// update the scene
		roomScene->moveTempModels(session);
		roomScene->ControllerActions(session);
        ...
        if (sessionStatus.IsVisible)
        {
            ...
            for (int eye = 0; eye < 2; ++eye)
            {
				...
				//Render avatar
				ovrPosef eye_glm = EyeRenderPose[eye];
				avatarClient->Render(eye_glm, proj);
